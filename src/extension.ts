...
export function activate(context: vscode.ExtensionContext) {
  console.log(
    'Congratulations, your extension "openaicodexcodeautocomplete" is now active!'
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("openaicodex.startSession", async () => {
      activated = true;
      ...
      if (!apiKey || !context.workspaceState.get("@key.verified", true)) {
        ...
        if (key === undefined) {
          vscode.window.showErrorMessage(
            "The key entered is empty or invalid, please try again.."
          );
          return;
        } else {
  ...
  function suggestCode(
    ...
    const data = new TextEncoder().encode(
      JSON.stringify({
        ...
        frequency_penalty: context.workspaceState.get(
          "@codex.frequency_penalty",
          0
        ),
        presence_penalty: context.workspaceState.get(
          "@codex.presence_penalty",
          0
        ),
      })
    );
    ...
    const handler = (res: any, resolve: any, reject: any) => {
      try {
        ...
        while (queueLock) {}
        queueLock = true;
        const index = queuedGeneration.findIndex(
          (val) => val === openDoc.fileName
        );
        if (index !== -1) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "openaicodex.suggestAutoComplete",
      async () => {
        if (!activated) {
          vscode.window.showErrorMessage("The session is not activated");
          await vscode.commands.executeCommand("openaicodex.startSession");
        }
        ...
        if (openDoc !== undefined && editor !== undefined) {
          ...
          queueLock = true;
          if (
            queuedGeneration.findIndex((val) => val === openDoc.fileName) !== -1
          ) {
            vscode.window.showErrorMessage(
              "Code generation is already running."
            );
            queueLock = false;
            return;
          } else {
          ...
          suggestCode(
            ...
            new vscode.Position(
              openDoc.lineCount - 1,
              openDoc.lineAt(openDoc.lineCount - 1).range.end.character
            )
          );
        } else {
          return;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("openaicodex.changeKey", async () => {
      const key = await vscode.window.showInputBox({
        password: true,
        title: "CODEX API key",
        prompt: "Enter API Key here...",
      });
      // vscode.window.showInformationMessage("Change OpenAI Codex API KEY!");
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("openaicodex.showSettings", async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("openaicodex.changeSettings", async () => {
      ...
      if (selected === "temperature") {
        ...
        if (num) {
          if (num < 0 || num > 1) {
            vscode.window.showWarningMessage(
              `Codex:Invalid Temperature entered ${num}`
            );
          } else {
        ...
        if (num) {
          if (num > 0 && num <= 4096) {
            ...
            vscode.window.showWarningMessage(
              `Codex: Invalid value for max_tokens ${num}`
            );
          }
        }
      } else if (selected === "top_p") {
        ...
        if (num) {
          if (num >= 0 && num <= 1) {
            ...
            vscode.window.showWarningMessage(
              `Codex: Invalid value for top_p ${num}`
            );
          }
        }
      } else if (selected === "frequency_penalty") {
        ...
        if (num) {
          if (num >= 0 && num <= 2) {
            ...
            vscode.window.showWarningMessage(
              `Codex: Invalid value for frequency_penalty ${num}`
            );
          }
        }
      } else if (selected === "presence_penalty") {
        ...
        if (num) {
          if (num >= 0 && num <= 2) {
            ...
            vscode.window.showWarningMessage(
              `Codex: Invalid value for presence_penalty ${num}`
            );
          }
        }
      }
